...
)
{
	...
	if ((UINT)*path < ' ') {				/* Null path name is the origin directory itself */
		...
		for (;;) {
//			printf("follow_path [%c%c%c%c%c%c%c%c.%c%c%c]\n",
//					path[0], path[1], path[2], path[3], path[4], path[5], path[6], path[7], path[8], path[9], path[10]);
